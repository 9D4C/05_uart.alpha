...
int fd; 	//put in main is ok too (30 2 32)
int ret; 	//
char c; 	//for text in the while 
...
// {
// 	int nwrite; 											// count
// 	float buf[64]={0xff,0xfe,1};								// start
...
//  buf+6=((data[0]+data[1])& 0xff);
// 	buf+7=0xa;
// 	buf+8=0xd;

// 	nwrite = write(fd, buf, 8*sizeof(float));
